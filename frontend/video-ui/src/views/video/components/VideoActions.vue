<template>
  <div class="side-actions" :class="{ 'hidden': isCleanMode }">
    <!-- 头像 -->
    <div class="action-item avatar-item" :class="{ active: activePanel === 'user' }">
      <button
        type="button"
        class="avatar-link"
        ...
        aria-label="查看作者信息"
        title="作者信息"
        @click.stop="$emit('toggle-user-panel')"
      >
    ...
    <!-- 点赞 -->
    <button type="button" class="action-item" :class="{ active: isLiked }" :aria-pressed="isLiked" :aria-label="isLiked ? '取消点赞' : '点赞'" title="点赞" @click="$emit('toggle-like')">
    ...
    <!-- 点踩 -->
    <button type="button" class="action-item" :class="{ active: isDisliked }" :aria-pressed="isDisliked" :aria-label="isDisliked ? '取消不喜欢' : '不喜欢'" title="不喜欢" @click="$emit('toggle-dislike')">
      <div class="action-icon">
        <svg viewBox="0 0 1024 1024">
    ...
    <!-- 评论 -->
    <button type="button" class="action-item" :class="{ active: activePanel === 'comments' }" :aria-pressed="activePanel === 'comments'" aria-label="打开评论" title="评论" @click.stop="$emit('toggle-comment-panel')">
      <div class="action-icon">
        <svg viewBox="0 0 1024 1024">
    ...
    <!-- 收藏 -->
    <button type="button" class="action-item" :class="{ active: isCollected }" :aria-pressed="isCollected" :aria-label="isCollected ? '取消收藏' : '收藏'" title="收藏" @click="$emit('toggle-collect')">
      <div class="action-icon">
        <svg viewBox="0 0 1024 1024">
    ...
    <!-- 分享 -->
    <button type="button" class="action-item" aria-label="分享视频" title="分享" @click="$emit('share')">
      <div class="action-icon">
        <svg viewBox="0 0 1024 1024">
